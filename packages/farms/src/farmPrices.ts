...
// either via direct calculation if farm is X-MCOIN or X-BUSD
// or via quoteTokenFarm which is quoteToken-MCOIN or quoteToken-BUSD farm
export const getFarmBaseTokenPrice = (
  ...
  // We can only calculate profits without a quoteTokenFarm for BUSD/MCOIN farms
  if (!quoteTokenFarm) {
    return FIXED_ZERO
  }
  ...
  // i.e. for farm PNT - pBTC we use the pBTC farm's quote token - MCOIN, (pBTC - MCOIN)
  // from the MCOIN - pBTC price, we can calculate the PNT - BUSD price
