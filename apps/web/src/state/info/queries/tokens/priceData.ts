...
const fetchTokenPriceData = async (
  ...
  try {
    ...
    // format token MCOIN price results
    const tokenPrices: {
      timestamp: string
      derivedBNB: number
      priceUSD: number
    ...
    // Get Token prices in MCOIN
    Object.keys(prices).forEach((priceKey) => {
      const timestamp = priceKey.split('t')[1]
      // if its MCOIN price e.g. `b123` split('t')[1] will be undefined and skip MCOIN price entry
      if (timestamp) {
        tokenPrices.push({
          timestamp,
    ...
    // Go through MCOIN USD prices and calculate Token price based on it
    Object.keys(prices).forEach((priceKey) => {
