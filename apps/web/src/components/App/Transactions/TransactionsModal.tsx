...
function renderTransactions(
  ...
  return (
    <Flex flexDirection="column">
      {transactions.map((tx) => {
        return (
          <Transaction key={tx.hash + tx.addedTime} tx={tx} chainId={chainId} />
        );
      })}
    </Flex>
  );
          ...
          <ModalBody>
            {hasTransactions ? (
              <>
                ...
                {Object.entries(sortedRecentTransactions).map(
                  ([chainId, transactions]) => {
                    const chainIdNumber = Number(chainId);
                    const groupedTransactions = groupBy(
                      Object.values(transactions),
                      (trxDetails) => Boolean(trxDetails.receipt)
                    );
                    ...
                    return (
                      <div key={`transactions#${chainIdNumber}`}>
                        <Text fontSize="12px" color="textSubtle" mb="4px">
                          {chains.find((c) => c.id === chainIdNumber)?.name ??
                            "Unknown network"}
                        </Text>
