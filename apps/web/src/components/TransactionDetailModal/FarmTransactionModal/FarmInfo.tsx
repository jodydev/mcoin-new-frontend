...
const FarmResult: React.FC<React.PropsWithChildren<FarmInfoProps>> = ({ pickedData }) => {
  ...
  const { targetRef, tooltip, tooltipVisible } = useTooltip(
    <Flex flexDirection="column">
      <ListStyle>{t('You have received 0.0005 MCOIN as a first-time MCOIN Smart Chain user')}</ListStyle>
      <ListStyle>
        {t('You can swap more MCOIN on')}
    ...
    <Box mb="24px">
      {isFirstStepError ? (
        ...
        <Box>
          ...
          <Text as="span">{t('did not succeed on the MCOIN Chain side. Please copy the')}</Text>
          <Text bold as="span" m="0 4px">
            {t('Transaction ID')}
          </Text>
