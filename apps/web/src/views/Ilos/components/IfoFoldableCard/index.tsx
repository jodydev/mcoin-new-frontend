...
const FoldableContent = styled.div<{ isActive: boolean }>`
  display: block;
  background: ${({ isActive, theme }) => (isActive ? theme.colors.darkColor : theme.colors.primary)};
`
      ...
      <FoldableContent isActive={publicIfoData.status !== -1 && publicIfoData.status !== 2}>
        ...
        <StyledCardBody>
          ...
          <div style={{ textAlign: "center" }}>
            <p>Note: To Purchase {publicIfoData.symbol} Directly, there will be three transaction confirmations that you will need to approve on Metamask. Please Approve all 3 for MCOIN purchases. Once these transactions are Approved, the deal will be completed. Once you have successfully completed the purchase of {publicIfoData.symbol} Tokens and approved all three transactions on your metamask wallet, You will receive your {publicIfoData.symbol} Tokens directly to the wallet address you used in the purchase. Thank you and Welcome to {publicIfoData.symbol} Town!</p>
            <br />
            <h1 style={{ fontSize: "larger" }}>{publicIfoData._name}</h1>
            {
              /*  publicIfoData.maxUserAmount.toString()==="0"?<h1>No Limit</h1>:<div><h1>Limit</h1>&nbsp;{getFullDisplayBalance(new BigNumber(publicIfoData.maxUserAmount),18,2).concat(" MCOIN per User")}</div>  */
            }
          </div>
          <br />
